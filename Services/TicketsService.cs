...
namespace EXPEDIT.License.Services {
    ...
    public class LicenseService : ILicenseService {
        ...
        public IEnumerable<IHtmlString> GetSearchResults(string text = null, Guid? supplierModelID = null, int? startRowIndex = null, int? pageSize = null)
        {
            ...
            using (new TransactionScope(TransactionScopeOption.Suppress))
            {
                ...
                return (from o in d.E_SP_GetSecuredSearch(text, contact, application, null, startRowIndex, pageSize, verified, found)
                        select GetSearchResultShape(new LicenseViewModel
                        {
        ...
        [Shape]
        public IHtmlString GetSearchResultShape(LicenseViewModel model)
        {
            return new HtmlString(string.Format("<a href='/License/go/{0}'><h2>{1}</h2>{2}</a>", model.UrlInternal, model.Title, model.Description)); //TODO:Extend search different object types
        }
